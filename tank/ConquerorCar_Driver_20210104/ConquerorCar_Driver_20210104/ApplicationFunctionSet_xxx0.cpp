 ...
 * @LastEditTime: 2021-01-04 17:20:19
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == ObstacleAvoidance_mode)
  {
    ...
    else
    {
...
#endif
      {
        ...
        {
          //获取时间戳 timestamp
          if (en == false || _millis() - timestamp > 3100)
          {
            en = true;
            timestamp = _millis();
          }
          if (function_xxx((_millis() - timestamp), 0, 500)) //-
          {
            is_ULTRASONIC_Get = 1;
            ApplicationFunctionSet_ConquerorCarMotionControl(Right, 250);
          }
          else if (function_xxx((_millis() - timestamp), 500, 600))
          {
            is_ULTRASONIC_Get = 2;
          }
          else if (function_xxx((_millis() - timestamp), 600, 1600)) //+
          {
            ApplicationFunctionSet_ConquerorCarMotionControl(Left, 250);
          }
          else if (function_xxx((_millis() - timestamp), 1600, 1700))
          {
            is_ULTRASONIC_Get = 3;
          }
          else if (function_xxx((_millis() - timestamp), 1700, 2200)) //0
          {
            ApplicationFunctionSet_ConquerorCarMotionControl(Right, 250);
          }
          else if (function_xxx((_millis() - timestamp), 2200, 2700)) //
          {
            ApplicationFunctionSet_ConquerorCarMotionControl(Backward, 250);
          }
          else if (function_xxx((_millis() - timestamp), 2700, 3000)) //0
          {
            ApplicationFunctionSet_ConquerorCarMotionControl(Left, 250);
          }
          else if (function_xxx((_millis() - timestamp), 3000, 3100)) //0
          {
            is_ULTRASONIC_Get = 4;
          }
        }
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == Follow_mode)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (function_xxx((_millis() - timestamp), 0, 500) || function_xxx((_millis() - timestamp), 1000, 1500) || function_xxx((_millis() - timestamp), 2000, 2500) || function_xxx((_millis() - timestamp), 3000, 3500)) //
        {
          ApplicationFunctionSet_ConquerorCarMotionControl(stop_it, 0);
        }
        else if (function_xxx((_millis() - timestamp), 500, 1000)) //-
        {
          ApplicationFunctionSet_ConquerorCarMotionControl(Right, 255);
        }
        else if (function_xxx((_millis() - timestamp), 1500, 2000) || function_xxx((_millis() - timestamp), 2500, 3000)) //+
        {
          ApplicationFunctionSet_ConquerorCarMotionControl(Left, 255);
        }
        else if (function_xxx((_millis() - timestamp), 3500, 4000)) //+
        {
          ApplicationFunctionSet_ConquerorCarMotionControl(Right, 255);
        }
        else if (function_xxx((_millis() - timestamp), 4000, 4500)) //+
        {
          ApplicationFunctionSet_ConquerorCarMotionControl(stop_it, 0);
        }
      }
...
void ApplicationFunctionSet::CMD_CarControlTimeLimit_xxx0(uint8_t is_CarDirection, uint8_t is_CarSpeed, uint32_t is_Timer)
{
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_CarControl_TimeLimit) //进入车子有时间限定控制模式
  {
    ...
    {
      if ((_millis() - Application_ConquerorCarxxx0.CMD_CarControl_Millis) > (is_Timer)) //判断时间戳
      {
        CarControl_TE = true;
        ApplicationFunctionSet_ConquerorCarMotionControl(stop_it, 0);
...
void ApplicationFunctionSet::CMD_CarControlTimeLimit_xxx0(void)
{
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_CarControl_TimeLimit) //进入车子有时间限定控制模式
  {
    ...
    {
      if ((_millis() - Application_ConquerorCarxxx0.CMD_CarControl_Millis) > (CMD_is_CarTimer)) //判断时间戳
      {
        CarControl_TE = true;
        ApplicationFunctionSet_ConquerorCarMotionControl(stop_it, 0);
                                                               ...
                                                               uint32_t is_LightingTimer)
{
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_LightingControl_TimeLimit) //进入灯光有时间限定控制模式
  {
    ...
    {
      if ((_millis() - Application_ConquerorCarxxx0.CMD_LightingControl_Millis) > (is_LightingTimer)) //判断时间戳
      {
...
void ApplicationFunctionSet::CMD_LightingControlTimeLimit_xxx0(void)
{
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_LightingControl_TimeLimit) //进入灯光有时间限定控制模式
  {
    ...
    {
      if ((_millis() - Application_ConquerorCarxxx0.CMD_LightingControl_Millis) > (CMD_is_LightingTimer)) //判断时间戳
      {
        LightingControl_TE = true;
        FastLED.clear(true);
        Application_ConquerorCarxxx0.Functional_Mode = CMD_Programming_mode; /*进入编程模式提示符<等待下一组控制命令的到来>*/
  ...
  if (c == '}') //数据帧尾部校验
  {
    ...
    else if (!error) //检查反序列化是否成功
    {
      ...
      case 2:                                                                    /*<命令：N 2> */
        ...
        Application_ConquerorCarxxx0.CMD_CarControl_Millis = _millis();
#if _is_print
        //Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
        break;
      ...
      case 7:                                                                         /*<命令：N 7> */
        ...
        Application_ConquerorCarxxx0.CMD_LightingControl_Millis = _millis();
#if _is_print
        //Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
        break;
